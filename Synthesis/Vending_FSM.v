...
module vending_top #(
  ...
  parameter [3:0] INIT_INV    = 4'd8,   // each product inventory (0..15)
  ...
  input  wire [1:0]  coin_type,          // (2): 00=$5, 01=$10, 10=$25
  ...
  output reg  [1:0]  change_type,         // (2): 00=$5,01=$10,10=$25
  output reg  [1:0]  status_code,        // (2) 00=OK,01=BUSY,10=ERROR (example)
  ...
  reg [5:0] change_bank;    // (6) in $5 units (total change capacity)
  reg [5:0] change_rem;     // (6) remaining change to return
  reg [1:0] sel_reg;        // (2) latched selection

  reg [5:0] price [0:3];    // (6) each
  reg [3:0] inv   [0:3];    // (4) each
  ...
  // Coin value decode (in $5 units)
  wire [5:0] coin_value =
      (coin_type==2'b00) ? 6'd1 :
      (coin_type==2'b01) ? 6'd2 :
      (coin_type==2'b10) ? 6'd5 : 6'd0;

  // Change can-subtract conditions (greedy)
  ...
  reg change_start_pulse;     // starts change FSM (1-cycle)
  ...
  always @(negedge clk or negedge rst_n) begin
    if (!rst_n) begin
      ...
      case (cstate)
        ...
        C_DONE: begin
          chg_done_pulse <= 1'b1; // tells main FSM
          status_code    <= 2'b00;
          do_clear_balance <= 1'b1;
        end
        default: ;
